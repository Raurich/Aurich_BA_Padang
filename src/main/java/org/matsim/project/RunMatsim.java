...
public class RunMatsim{
	...
	public final static boolean DECONGESTION = true;
...
//			decongestionSettings.setDecongestionApproach(DecongestionApproach.P_MC);
			...
			decongestionSettings.setKp(0.0025);
		...
		config.strategy().setFractionOfIterationsToDisableInnovation(0.8); //nach x% der Iterationen Rerouting ausschalten
		...
		controler.addOverridingModule(new AbstractModule(){
			...
			public void install() {
				...
				this.bind(EvacAgents.class).asEagerSingleton();
			
				this.addEventHandlerBinding().to(EvacAgents.class);
				this.addControlerListenerBinding().to(EvacAgents.class);
			}
